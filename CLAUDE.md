...
| `pictures/` | Patent drawing images referenced by individual entries' `img` or `imgs[]` field. Living folder at https://github.com/postmillennium-MTB/MTB-PATENT-ATLAS/tree/main/pictures. Two naming patterns coexist: `US<number><kindcode>.png` (e.g. `US7665929B2.png` — the majority pattern) and a few bare-number files from an earlier pass (`9102378.png`). Prefer the full `US<number><kind>.png` form for anything new. **To add a new image:** (1) upload directly via GitHub's web UI to this folder (or via `git add pictures/US<num><kind>.png && git commit`); (2) add corresponding `img` (single) or `imgs[]` (multiple) field to the entry in `index.html` with bilingual `imgAlt` describing the figure; (3) run verification script; (4) commit index.html change. |
