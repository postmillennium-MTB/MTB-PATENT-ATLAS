...
Every filter combination, search term, active tab, and individual card has its own URL — the browser's address bar updates automatically as you filter and browse, and reloading or bookmarking a page returns to the same view. Individual cards resolve by patent number (or a title slug for unverified entries), so a URL can point at one specific patent. There's still no generic "copy link" button for this (see *Recent updates* below for why one was pulled) — the 🔗 Share menu covers posting to a specific platform, not copying the raw URL, so grabbing the bare address bar link is still a manual copy.

### 🔗 Share menu
A "🔗 Share" button next to Surprise Me opens a small dropdown with one-click share-outs to X, Facebook, and Reddit. Each opens that platform's own share-intent page in a popup, pre-filled with the *current* URL — including whatever filters, search term, or open card are active, since the URL always reflects the live view (see above). No clipboard permission, no Web Share API, and no added dependency: these are plain `https://x.com/intent/...`-style links, native `<details>` for the dropdown (same zero-JS-to-open pattern as the "All N patents on Google Patents" disclosure on multi-patent cards), and the menu closes itself after a click, on Escape, or on an outside click.
...
- **Re-verified the Castellano "Sweet Spot" Unified Rear Triangle entry** (US 5,474,318) against Google Patents — it had a correct, specific patent number all along, but `conf` was still sitting at `"m"`, which renders the "patent number unverified" badge regardless of whether a number is actually present. Confirmed the grant date (Dec 12, 1995) matches the card exactly and bumped it to `"v"`. Picked up three more licensees in the same pass — WTB, Rocky Mountain, and Joe Breeze's own bikes.
- **Added a 🔗 Share menu** — X, Facebook, and Reddit share-intent links in a small dropdown next to Surprise Me, each pre-filled with whatever's currently on screen (filters, search, or an open card), since the URL already reflects the live view via the existing deep-link mechanism. Deliberately *not* a generic "copy link" button or a Web Share API call — see the *Share scope* discussion this was built from: a prior pass had already built and then deliberately removed a copy-link button ("revisited at a later date"); this is a narrower, platform-specific answer to the same request rather than a resurrection of that button. Verified interactively with Playwright (open/close via click, outside-click, and Escape; correct intent URLs built per platform; renders correctly across all four club themes and at mobile width) rather than just eyeballed. Also fixed a stale `228+` patent count in the page's `<meta>`/OG/Twitter-card tags to the real `243`.
