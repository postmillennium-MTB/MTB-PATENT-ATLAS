...
### 🔗 URL reflects the current view — and two buttons to share it
Every filter combination, search term, active tab, and individual card has its own URL — the browser's address bar updates automatically as you filter and browse, and reloading or bookmarking a page returns to the same view. Individual cards resolve by patent number (or a title slug for unverified entries), so a URL can point at one specific patent. Two buttons put that mechanism one click away: a **🔗 Share view** button in the filter toolbar copies a link to the current filtered/searched view, and every card carries its own small send-icon **copy-link button** beside the expand/collapse toggle, which copies a direct link to that one entry regardless of what's currently filtered. Both fall back through clipboard → `execCommand` → a manual-copy prompt, since the Clipboard API is frequently blocked inside the cross-origin iframe this tool is designed to run in.
...
- **Verified the Castellano Sweet Spot entry (US 5,474,318)** — it had the correct number all along, but a stale `conf: "m"` tier meant the card still rendered the "patent number unverified" badge. Cross-checked against Google Patents (granted Dec 12, 1995, matching the card's existing data exactly) and bumped to `v`. Same root cause, same fix, on the **BMX Gyro** entry (US 6,968,927 + 5,829,314 + 6,164,153 + 6,205,635, Brian Scura): all four numbers were already correct, `conf` just hadn't been updated after they were confirmed — corrected to `v`, filled in the previously-`null` grant year (2000, confirmed from a newly-added patent-drawing image, `US6164153A.png`), and wired that image into the card.
- **Restored the "Share view" toolbar button** (copy the current filter/search/tab state as a URL — the underlying `syncHash()` mechanism was never removed, only the button) with its original clipboard-then-execCommand-then-prompt fallback chain, since this tool runs inside cross-origin iframes where the Clipboard API is often blocked. **Added a new per-card share button** — a small icon-only button next to each card's expand/collapse toggle that copies a direct `#p=<patent>` deep link to that one entry. Required restructuring `.chead` (itself a `<button>`) and the new button into `.chead-row` flex siblings, since buttons can't nest. Both buttons share one `copyToClipboard()` helper so the fallback logic lives in exactly one place.
